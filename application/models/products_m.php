...
class Products_m extends CI_Model{
  ...
  function get_json_competitor($q){
     ...
     /* $arrOE = $this->get_json_oemnuevo($q,TRUE);
      ...
      # busca en smp
      $arrSMP = $this->get_json_smpnuevo($q,TRUE);
      if(is_array($arrSMP)){
          $arrDatos = array_merge($arrDatos,$arrSMP);
      }
      ...
      # busca en wells
      $arrWells = $this->get_json_wellsnuevo($q,TRUE);
      if(is_array($arrWells)){
          $arrDatos = array_merge($arrDatos,$arrWells);
      }
  ...
  function get_json_oemnuevo($q,$return_array=FALSE){
      $this->db_search->select('oem.nombre AS oem')
  ...
  function get_json_smpnuevo($q,$return_array=FALSE){
      $this->db_search->select('smp.nombre AS smp')
  ...
  function get_json_wellsnuevo($q,$return_array=FALSE){
      $this->db_search->select('wells.nombre AS wells')

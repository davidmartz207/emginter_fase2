...
class Products extends CI_Controller{
    public function __construct(){
        ...
        $this->arrDatos['title']        = '';
        $this->arrDatos['vista']        = 'products_v';
    ...
    public function index(){
        ...
        if($this->input->post('btSearchProductType') or
           ...
           $this->arrDatos['send_post'] = TRUE;
            ...
            $SESS_linea = $this->modelo->get_field('linea');
        }


        #linea
        if($this->input->post('linea')||!isset($SESS_linea)){
            $this->arrDatos['linea'] = $this->input->post('linea');
            $this->modelo->set_field('linea',$this->input->post('linea'));
        }
        else{
            $this->arrDatos['linea'] = $SESS_linea;
        }

        $this->arrDatos['session']=$sesion_data["id_usuario"];
        ...
        if($this->input->post('btSearchProductType') or 
           $SESS_searcht=='btSearchProductType' or 
           !empty($sess_id_product_type)){

            $search_by = "1.- Product Type. sess_id_product_type: $sess_id_product_type";
            $type      = 'default_search';

            $this->_clear_form2();
            $this->_clear_form3();
            $this->_clear_form4();
            $this->modelo->set_field('SESS_searcht','btSearchProductType');
            $this->arrDatos['SESS_searcht'] = 'btSearchProductType';

            $txtProductType = $this->modelo->get_field('selTiposProductosDownloads');
            if($this->input->post('selTiposProductosDownloads') or 
               ($txtProductType and !$sess_id_product_type)){
                
                $this->modelo->set_field('sess_id_product_type','');
                $arrParam['tipo_producto'] = $this->input->post('selTiposProductosDownloads');

                if(empty($arrParam['tipo_producto'])){
                    $arrParam['tipo_producto'] = $this->modelo->get_field('selTiposProductosDownloads');
                }else{
                    $this->modelo->set_field('selTiposProductosDownloads',$arrParam['tipo_producto']);
                }
                $arrParam['tipo_producto']                    = (int)$arrParam['tipo_producto'];
                $this->arrDatos['selTiposProductosDownloads'] = $arrParam['tipo_producto'];
                $this->arrDatos['selTiposProductos']          = $arrParam['tipo_producto'];
                
            }elseif($sess_id_product_type){
                $arrParam['tipo_producto']                    = $sess_id_product_type;
                $this->arrDatos['selTiposProductosDownloads'] = $arrParam['tipo_producto'];
                $this->arrDatos['selTiposProductos']          = $arrParam['tipo_producto'];
                $this->modelo->set_field('selTiposProductosDownloads',$arrParam['tipo_producto']);
            }
        ...
        # ----------------------------------------------------------------------
        # 2.- Buscar por Número Parte EMG
        }elseif($this->input->post('btSearchSKU') or $SESS_searcht=='btSearchSKU'){
            ...
            $this->_clear_form3();
            $this->_clear_form4();
            $this->modelo->set_field('SESS_searcht','btSearchSKU');
			$this->arrDatos['SESS_searcht'] = 'btSearchSKU';

            $txtSKU = $this->modelo->get_field('txtSKU');

            if($this->input->post('txtSKU') or $txtSKU){
                $arrParam['sku']          = $this->input->post('txtSKU')?:$txtSKU;
                $this->arrDatos['txtSKU'] = html_escape($arrParam['sku']);
                $this->modelo->set_field('txtSKU',$this->arrDatos['txtSKU']);
            }else{
                $this->arrDatos['msjError'] = lang('field_sku_placeholder');
                $error = TRUE;
            }
            
        # ----------------------------------------------------------------------
        # 3.- Buscar por Número de Intercambio de Competidor
        }elseif($this->input->post('btSearchCompetitor') or $SESS_searcht=='btSearchCompetitor'){

            $search_by = '3.- Competitor';
            $type      = 'default_search';
            
            $this->_clear_form1();
            $this->_clear_form2();
            $this->_clear_form4();
            $this->modelo->set_field('SESS_searcht','btSearchCompetitor');
			$this->arrDatos['SESS_searcht'] = 'btSearchCompetitor';
            
            #-------------------------------------------------------------------
            $txtCompetitor                   = $this->input->post('txtCompetitor');
            if(empty($txtCompetitor)){
                $txtCompetitor = $this->modelo->get_field('txtCompetitor');
            }else{
                $this->modelo->set_field('txtCompetitor',$txtCompetitor);
            }
            $this->arrDatos['txtCompetitor'] = html_escape($txtCompetitor); 
            #-------------------------------------------------------------------

            if(!empty($txtCompetitor)){
                $arrParam['oemSmpWells'] = $txtCompetitor;
                #---------------------------------------------------------------
               /* $hddCompetitor = $this->input->post('selCompetitor');
                if(empty($hddCompetitor)){
                    $hddCompetitor = $this->modelo->get_field('selCompetitor');
                }else{
                    $this->modelo->set_field('selCompetitor',$hddCompetitor);
                }
                $this->arrDatos['selCompetitor'] = $hddCompetitor;*/
                #---------------------------------------------------------------
                ...
                $this->arrDatos['msjError'] = lang('field_competitor_placeholder');
                $error = TRUE;
            }           
        # ----------------------------------------------------------------------
        # 4.- Buscar por Vehículo
        }elseif($this->input->post('btSearchPpal') or $SESS_searcht=='btSearchPpal'){
        ...
        if(!empty($type)){
            ...
            if($this->input->post('linea')||!isset($SESS_linea)){
                $arrParam['linea'] = $this->input->post('linea');
                $this->modelo->set_field('linea',$this->input->post('linea'));
            }
            else{
                $arrParam['linea'] = $SESS_linea;
            }
    ...
    public function get_json_sku($linea = null){
        $q = isset($_GET["term"]) ? $_GET["term"] : "";
        if(!empty($q)){
            echo $this->modelo->get_json_sku($q,$linea);
        }
    }
    
    public function get_json_competitor($linea = null){
        $q = isset($_GET["term"]) ? $_GET["term"] : "";
        if(!empty($q)){
            echo $this->modelo->get_json_competitor($q,$linea);
        }
    }
    ...
    public function get_json_all_products_type($linea = null){
        echo get_json_tipos_productos($linea);
    }
    ...
    function get_json_product_type_name_by_sku(){
        $sku = $this->input->post('txtSKU');
        echo $this->modelo->get_json_product_type_name_by_sku($sku);
    }
    function get_json_product_type_name_by_custo(){
        $sku = $this->input->post('custo');
        echo $this->modelo->get_json_product_type_name_by_custo($sku);
    }
}

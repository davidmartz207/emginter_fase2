...
class Panel_products extends CI_Controller{
    ...
    public function index($pag=0){
        //echo "pagina: ".$pag."<pre>",print_r($_POST),"</pre>";
    ...
    public function export(){
        ...
        if(is_array($arrDatos)){
            ...
            exit;
                ...
                $this->modelo->set_field('result',4);
                redirect('panel_products');
        }
        # is_array
    }
        ...
        foreach ($objWorksheet->getRowIterator() as $fila => $row){
            ...
            foreach ($cellIterator as $columna => $cell){
                ...
                if($fila == 1){
                    ...
                    if(isset($arrFormatoCamposExcel[$columna]) and 
                       $arrFormatoCamposExcel[$columna] <> $celda){
                        $Formato_correcto = FALSE;
                        $arrProceso['msjError'] = 'Se ha detectado que el formato del XLS es incorrecto. '.$fila_columna;
                        $arrProceso['process']  = FALSE;
                        break(2);
                    }
                    continue;
                ...
                if($columna==0){         
                    ...
                    if(empty($arrProduct['oem'])){
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El OEM es obligatorio. '.$fila_columna;
                        break(2);
                    }
                ...
                # product type ingles
                    ...
                    if(empty($arrProduct['product_type_en'])){
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El tipo de producto (Ingles) es obligatorio. '.$fila_columna;
                        break(2);
                    }

                # product type español
                }elseif($columna==3){
                    //echo $celda."..".mb_detect_encoding($celda);
                    $arrProduct['product_type_es'] = formatear_celda($celda);
                    //echo " -- ".$arrProduct['product_type_es']."..".mb_detect_encoding($arrProduct['product_type_es']);exit;
                    if(empty($arrProduct['product_type_es'])){                        
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El tipo de producto (Español) es obligatorio. '.$fila_columna;
                        break(2);
                    }else{
                ...
                # sku
                    ...
                    if(empty($arrProduct['sku'])){
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El código SKU es obligatorio. '.$fila_columna;
                        continue;
                    }
                ...
                # marca
                    ...
                    $arrProduct['marca'] = formatear_celda($celda);
                    if(empty($arrProduct['marca'])){
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El campo Marca es obligatorio. '.$fila_columna;
                        break(2);
                    }else{
                ...
                # modelo
                    ...
                    if(empty($arrProduct['modelo'])){
                        $arrProceso['process']  = FALSE;
                        $arrProceso['msjError'] = 'El campo Modelo es obligatorio. '.$fila_columna;
                        break(2);
                    }else{
                ...
                # engine type
                    ...
                    if(empty($arrProduct['tipo_motor'])){
                        $$arrProceso['process'] = FALSE;
                        $arrProceso['msjError'] = 'El campo Tipo de motor es obligatorio. '.$fila_columna;
                        break(2);
                    }else{
            ...
            if($arrProceso['process'] == TRUE){
                ...
                if(!empty($id_producto)){
                    ...
                    # no es necesario--if(!$this->modelo->existe_aplicacion_producto($id_producto,$arrProduct['id_tipo_motor'],$arrProduct['id_marca'],$arrProduct['id_modelo'])){
    ...
    public function get_json_modelos($id=''){
        $id = (int)$id;

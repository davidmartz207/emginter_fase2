...
class Panel_gallery extends CI_Controller{
    public function __construct(){
        parent::__construct();
        $this->load->library(array('pagination','form_validation'));
        $this->load->model('panel_gallery_m','modelo');
        $this->arrDatos['title']     = 'Panel Products';
        $this->arrDatos['vista']     = 'panel_gallery_list_v';
    ...
    public function index(){
        $config['base_url']            = site_url('panel_gallery').'/index/';
        ...
        if(empty($selOrderBy) and $this->session->userdata('panel_gallery_order_by')){
            $selOrderBy = $this->session->userdata('panel_gallery_order_by');
        }else{
            $this->session->set_userdata('panel_gallery_order_by',$selOrderBy);
        }
        if(empty($selUpDown) and $this->session->userdata('panel_gallery_selUpDown')){
            $selUpDown = $this->session->userdata('panel_gallery_selUpDown');
        }else{
            $this->session->set_userdata('panel_gallery_selUpDown',$selUpDown);
        }
    ...
    public function ins(){
            ...
            # --- fin validaciones del archivo a subir..
                ...
                if($id=$this->modelo->registrar($this->arrDatos)){
                    ...
                    if($image_upload){
                        ...
                        // subimos la imagen
                        $image                    = procesar_imagenes($arrFILE,('top_gallery/'.$id.'/'),false,array('top_name'=>$top_name));
        ...
        $this->arrDatos['vista'] = 'panel_gallery_ins_v';
        $this->load->view('panel/includes/template',$this->arrDatos);
    }
    ...
    public function upd($data=''){        
       ...
       //echo '<pre>',print_r($_FILES),'</pre>';
        ...
        if(!empty($data)){
            ...
            if($this->input->post('btEditar')){
                ...
                # --- fin validaciones del archivo a subir..
                    ...
                    if($image_upload){
                        ...
                        $this->arrDatos['Imagen'] = procesar_imagenes($arrFILE,('top_gallery/'.$id.'/'),false,array('top_name'=>$top_name));
                    }

                    // actualizamos los datos
                    if($this->modelo->set_data($this->arrDatos)){
                        $this->session->set_userdata('controller','panel_gallery');
                        ...
                        redirect('panel_gallery/upd/'.$this->arrDatos['id']);
                    }else{
            ...
            $this->arrDatos['vista'] = 'panel_gallery_upd_v';

...
if ( ! function_exists('get_catalog_products'))
{
 function get_catalog_products(){
        ...
        $CI =& get_instance();
        $CI->load->database('default');
        $CI->db->select('id_catalogo,nombre_en,nombre_es,path_img,ruta_interna,ruta_externa')
...
{
   function get_json_tipos_productos(){
        $lang = get_lang();
        if($lang=='en'){
            $name = 'nombre_en';
        }elseif($lang=='es'){
        ...
        $CI =& get_instance();
        $CI->load->database('default');
        $query = $CI->db->query("SELECT id_tipo_producto AS id,nombre_en,nombre_es

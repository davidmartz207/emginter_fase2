...
class Panel_manufacturer extends CI_Controller{
    public function __construct(){
        parent::__construct();
        $this->load->library(array('pagination','form_validation'));
        $this->load->model('panel_manufacturer_m','modelo');
        $this->arrDatos['title']     = 'Panel Manufacturer';
        $this->arrDatos['vista']     = 'panel_manufacturer_list_v';
        $this->arrDatos['sMsjError'] = '';
    }
     
    public function index(){
        $config['base_url']            = site_url('panel_manufacturer').'/index/';
        ...
        if(empty($selOrderBy) and $this->session->userdata('panel_manufacturer_order_by')){
            $selOrderBy = $this->session->userdata('panel_manufacturer_order_by');
        }else{
            $this->session->set_userdata('panel_manufacturer_order_by',$selOrderBy);
        }
        if(empty($selUpDown) and $this->session->userdata('panel_manufacturer_selUpDown')){
            $selUpDown = $this->session->userdata('panel_manufacturer_selUpDown');
        }else{
            $this->session->set_userdata('panel_manufacturer_selUpDown',$selUpDown);
        }
    ...
    public function ins(){
        ...
        if($this->input->post('btSubmit')){
            ...
            if($this->form_validation->run() == FALSE){
                ...
                if($id=$this->modelo->registrar($this->arrDatos)){
                    ...
                    $this->session->set_userdata('id_manufacturer',$id);

                    redirect('panel_manufacturer');
                }else{
        ...
        $this->arrDatos['vista'] = 'panel_manufacturer_ins_v';
        $this->load->view('panel/includes/template',$this->arrDatos);
    }
    ...
    public function upd($data=''){        
       ...
       //echo '<pre>',print_r($_FILES),'</pre>';
        ...
        if(!empty($data)){
            ...
            if($this->input->post('btEditar')){
                ...
                if($this->form_validation->run() == FALSE){
                    ...
                    if($this->modelo->set_data($this->arrDatos)){
                        $this->session->set_userdata('controller','panel_manufacturer');
                        ...
                        redirect('panel_manufacturer/upd/'.$this->arrDatos['id']);
                    }else{
            ...
            $this->arrDatos['title'] = 'Edit Manufacturer';
            $this->arrDatos['vista'] = 'panel_manufacturer_upd_v';

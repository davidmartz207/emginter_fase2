...
class Login extends CI_Controller {
    ...
    public function conect($recover=false){
        ...
        if($this->input->post('btIngresar')){
            ...
            }elseif($this->modelo->consulta_cantidad($sEmail) >= 2){
                $this->arrDatos['sMsjError'] = lang('user_logged_in_other_instance');
                $this->modelo->consulta_sesion($sEmail);
            }

            if($this->arrDatos['sMsjError']){
                $this->index();
            }else{
                if($this->modelo->consulta_tipo($sEmail)!=1){
                    $this->modelo->contar($sEmail);
                }
